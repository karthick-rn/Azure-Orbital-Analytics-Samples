...
parser = argparse.ArgumentParser(
    description='Arguments required to run tiling function')
...
parser.add_argument('--src_folder_name', default=None,
                    required=True, help='Folder where the input data is stored')
parser.add_argument('--file_name', type=str, required=True,
                    help='Input file name to be tiled (with extension)')
...
def tile_img(input_path: str,
    ...
    try:
        ...
        for tile_touple in tile_combinations:
            ...
            print(
                f"Got Starting Coordinates - {x_start_point},{y_start_point}")
...
def process_img_folder(args):
    ...
    for img_name in os.listdir(args.path_to_input_img):
        ...
        tile_img(args.path_to_input_img, args.path_to_output,
                 img_name, args.tile_size)
...
if __name__ == "__main__":
    ...
    mssparkutils.fs.put(
        f'abfss://{args.storage_container}@{args.storage_account_name}.dfs.core.windows.net/tiles/__processing__.txt', 'started tiling ...', True)

    try:
        tile_img(input_path, output_path, args.file_name, args.tile_size)
        mssparkutils.fs.rm(
            f'abfss://{args.storage_container}@{args.storage_account_name}.dfs.core.windows.net/tiles/__processing__.txt', True)
    except:
        mssparkutils.fs.append(
            f'abfss://{args.storage_container}@{args.storage_account_name}.dfs.core.windows.net/tiles/__processing__.txt', 'tiling  errored out', True)
        raise
