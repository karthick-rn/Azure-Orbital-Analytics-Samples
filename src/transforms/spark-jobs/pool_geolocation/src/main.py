...
from utils import init_logger, parse_config

DEFAULT_CONFIG = {"probability_cutoff": 0.5,
                  "width": 512.1, "height": 512, "tag_name": "pool"}
...
parser = argparse.ArgumentParser(
    description='Arguments required to run pool geolocation function')
...
def get_pool_gelocations(input_path: str,
    ...
    for pam_file in glob.glob(f'{input_path}/*.aux.xml'):
        ...
        if tfm_xml is None:
            logger.error(
                f"could not find node 'GeoTransform' in file {pam_file} - quiting")
            exit(1)
        ...
        logger.info(
            f"using width: {width}, height: {height}, probability cut-off: {prob_cutoff}")
        logger.debug(
            f"looking for custom vision JSON files using `{input_path}/{img_name}*.json`")
        ...
        for json_path in glob.glob(f'{input_path}/{img_name}*.json'):
            ...
            predictions = [pred for pred in predictions["predictions"]
                           if pred["probability"] >= prob_cutoff and pred["tagName"] == tag_name]
            ...
            for pred in predictions:
                ...
                bbox = shp.geometry.Polygon(zip(
                    *tfmr.transform(*rio.transform.xy(tfm, *reversed(img_bbox.boundary.xy), offset="ul"))))
        ...
        if len(all_predictions) > 0:
            pools_geo = gpd.GeoDataFrame(
                all_predictions, geometry="boundingBox", crs=dst_crs)
            pools_geo["center"] = pools_geo.apply(lambda r: str(
                asarray(r["boundingBox"].centroid).tolist()), axis=1)
...
if __name__ == "__main__":
    ...
    mssparkutils.fs.put(
        f'abfss://{args.storage_container}@{args.storage_account_name}.dfs.core.windows.net/{dst_folder_name}/__processing__.txt', 'started tiling ...', True)
    ...
    try:
        ...
        mssparkutils.fs.rm(
            f'abfss://{args.storage_container}@{args.storage_account_name}.dfs.core.windows.net/{dst_folder_name}/__processing__.txt', True)
    except:
        # remove the placefolder file upon failed run
        mssparkutils.fs.append(
            f'abfss://{args.storage_container}@{args.storage_account_name}.dfs.core.windows.net/{dst_folder_name}/__processing__.txt', 'tiling  errored out', True)
        raise
