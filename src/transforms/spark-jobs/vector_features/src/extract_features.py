...
parser = argparse.ArgumentParser(
    description='Arguments required to run vector feature')
...
parser.add_argument('--storage_account_key', default=None, required=True,
                    help='Key to the storage account where the input data resides')
parser.add_argument('--storage_account_src_container', type=str,
                    required=True, help='Container under which the input data resides')
parser.add_argument('--storage_account_dst_container', default=None,
                    required=True, help='Container where the output data will be saved')
...
def extract_features_from_gpkg(storage_account_name: str, storage_account_key: str, storage_account_src_container: str, src_storage_folder: str, storage_account_dst_container: str, dst_storage_folder: str, file_name: str):
    ...
    kwargs = {'format': 'GTiff', 'dstSRS': '+proj=lcc +datum=WGS84 +lat_1=25 +lat_2=60 +lat_0=42.5 +lon_0=-100 +x_0=0 +y_0=0 +units=m +no_defs',
              'srcSRS': '+proj=longlat +datum=WGS84 +no_defs'}
    ds = gdal.Warp('output_warp.tif',
                   f'/vsiadls/{storage_account_src_container}/{src_storage_folder}/{file_name}', **kwargs)
